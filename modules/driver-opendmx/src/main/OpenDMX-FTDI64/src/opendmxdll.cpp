...
(JNIEnv *_enc, jclass _class, jint _mode) {
  ...
  if (_mode == OPENDMXUSB_MODE_TX) {
    if ((hDevice = FT_W32_CreateFile(buffer, GENERIC_WRITE, 0, NULL,
            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FT_OPEN_BY_DESCRIPTION,
            NULL)) == INVALID_HANDLE_VALUE)
      return false;
  } else {
    if ((hDevice = FT_W32_CreateFile(buffer, GENERIC_READ, 0, NULL,
            OPEN_EXISTING,
            FILE_ATTRIBUTE_NORMAL | FT_OPEN_BY_DESCRIPTION | FILE_FLAG_OVERLAPPED,
            NULL)) == INVALID_HANDLE_VALUE)
      return false;
  }
  ...
  if (_mode == OPENDMXUSB_MODE_TX) {
    pthread_create(&pThread, NULL, DmxReceive, &i);
  } else {
    pthread_create(&pThread, NULL, DmxTransmit, &i);
  }
...
void *(DmxTransmit) (void *) {
  ...
  while (1) {
    ...
    if (delay > 0) {
      target.QuadPart = (LONGLONG)
              (t.QuadPart + clockFrequency.QuadPart * (delay) / (double) 1000000);
    ...
    if (!FT_W32_WriteFile(hDevice, dmxBuffer, 513, &bytes, NULL)) {
      frameRate[framesToAverage - 1] = 0;
      LeaveCriticalSection(&cs);
      continue;
    }
